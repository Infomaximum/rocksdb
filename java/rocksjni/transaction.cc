...
// TODO(AR) consider refactoring to share this between here and rocksjni.cc
jbyteArray txn_get_helper_update(JNIEnv* env, const FnGetForUpdate &fn_get,
	const jlong &jread_options_handle, const jbyteArray &jkey,
	const jint &jkey_part_len, jboolean jexclusive) {
	auto* read_options =
		reinterpret_cast<rocksdb::ReadOptions*>(jread_options_handle);
	jbyte* key = env->GetByteArrayElements(jkey, nullptr);
	rocksdb::Slice key_slice(reinterpret_cast<char*>(key), jkey_part_len);
	...
	std::string value;
	rocksdb::Status s = fn_get(*read_options, key_slice, &value, jexclusive);
...
jbyteArray Java_org_rocksdb_Transaction_getForUpdate__JJ_3BIJ(JNIEnv* env,
  ...
  auto* column_family_handle =
        reinterpret_cast<rocksdb::ColumnFamilyHandle*>(jcolumn_family_handle);
  auto* txn = reinterpret_cast<rocksdb::Transaction*>(jhandle);
  FnGetForUpdate fn_get_for_update =
      std::bind<rocksdb::Status (rocksdb::Transaction::*) (const rocksdb::ReadOptions&, rocksdb::ColumnFamilyHandle*, const rocksdb::Slice&, std::string*, bool)>(
          ...
          _2, _3, _4);
  return txn_get_helper_update(env, fn_get_for_update, jread_options_handle, jkey,
      jkey_part_len, jexclusive);
}
...
jbyteArray Java_org_rocksdb_Transaction_getForUpdate__JJ_3BI(JNIEnv* env,
  ...
  FnGetForUpdate fn_get_for_update =
      std::bind<rocksdb::Status (rocksdb::Transaction::*) (const rocksdb::ReadOptions&, const rocksdb::Slice&, std::string*, bool)>(
          &rocksdb::Transaction::GetForUpdate, txn, _1, _2, _3, _4);
  return txn_get_helper_update(env, fn_get_for_update, jread_options_handle, jkey,
      jkey_part_len, jexclusive);
}
...
jbyte Java_org_rocksdb_Transaction_getExecutionStatus(JNIEnv* env,
  ...
  switch (exec_status) {
    case rocksdb::Transaction::STARTED:
      return 0x0;

    case rocksdb::Transaction::AWAITING_PREPARE:
      return 0x1;

    case rocksdb::Transaction::PREPARED:
      return 0x2;

    case rocksdb::Transaction::AWAITING_COMMIT:
      return 0x3;

    case rocksdb::Transaction::COMMITED:
      return 0x4;

    case rocksdb::Transaction::AWAITING_ROLLBACK:
      return 0x5;

    case rocksdb::Transaction::ROLLEDBACK:
      return 0x6;

    case rocksdb::Transaction::LOCKS_STOLEN:
      return 0x7;
  }
